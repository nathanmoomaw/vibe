...
- [ ] Restore a way to manually find planet glyphs on noise channels — the display-drag/knob used to adjust noise frequency (and the glyph-fade logic tracks noise frequency), but as of the Jul 13 2026 color-pairing change the knob morphs color instead and frequency is only set by readings/presets, so that exploratory "search by ear" interaction is gone for noise channels specifically. Sep 1 2026: decided against the dual-knob's outer ring for this (already claimed by volume) — will extend the circle-viz's existing global up/down=volume, left/right=rate drag instead, since left/right=rate is presently meaningless while isolated on a noise channel (noise has no "rate"), making it free to repurpose as frequency-search specifically during noise isolation. Not yet built
