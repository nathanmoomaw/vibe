...
- [ ] Stop button opacity, another pass — halved the rest-state alphas/waver range again on `dev/v1-stopupdate` (cut from `main`, not merged), pending local review before going live
...
- [ ] Lissajous-figure visualizer mode — per the Sep 3 2026 `/shapes` digest, a Lissajous curve (two perpendicular sine-driven x/y coordinates) is a literal picture of a frequency ratio between two active tones — cheap to render (no shader needed) and distinct from the existing spectrum-bar/Chladni/flower layers; consonant intervals would trace simple closed loops, dissonant ones sprawling/non-repeating ones
- [ ] Bouba/kiki-informed preset shape vocabulary — per the same digest, deliberately bias calming/relaxing/dreaming/floating toward rounded continuous-curve shapes and slow blob-like motion, vs. a more angular/faster-transient treatment for anything focus-adjacent, as an intentional (currently unused) visual lever independent of the audio underneath

## Completed
- [x] Astro-sign isolation no longer renders extra octave-sibling planet glyphs while isolated — circle-viz draw loop now gates the planet loop on the isolated sign so exactly one glyph shows until restored
- [x] Circle-viz faux-glass lens sheen (slow-breathing diagonal highlight) + cymatics layer extended ~75px past the ring with its own built-in falloff, instead of being clipped flush at the bezel
- [x] Astro chart (`VibeAstro.jsx`): classic aspect lines between planets in a major aspect (conjunction/sextile/square/trine/opposition), and dismissable by clicking anywhere outside the physical console, not just the same footer icon or the ×
- [x] New `/shapes` learn skill (shape/form-to-sound perception — bouba/kiki, Chladni's Law, Lissajous figures, motion/easing) registered into `/learn`'s roster; first digest saved, emailed, and synced to cat.obfusco.us
- [x] Tone rate-knob dragging no longer spams full-volume bell/chime/gong/birds strikes — `updateToneRate()` updates the interval in place with a throttled, quiet (35% vol, ≤1/220ms) preview strike instead of restarting via `startTone` on every pointermove. Also fixed a latent bug where the periodic-tone reschedule loop ignored live interval changes (stale closed-over `interval` instead of `state.interval`)
