...
**Larger + repositioned caption (Sep 10 2026):** `.vas__wheel` grew from `min(88vw,88vh)` to `min(96vw,96vh)` (then `99vw/99vh` on Sep 11, per follow-up feedback to push it even closer to the edges — stopped at 99 not 100 so the ring's own stroke, already inset ~10% of the SVG viewBox from its edge, never touches the literal viewport edge) — deliberately bigger than the console, so the ring sits further out from center and is less likely to visually crowd the physical console underneath even though the chart as a whole is bigger. The `present sky · lat/lon · time` caption moved out of the centered `.vas__wrap` flex column (removed, no longer needed with only the wheel left in it) into `position: fixed; bottom:14px; left:16px`, mirroring the close button's fixed top-right placement as a paired corner anchor instead of competing with the console for bottom-center.
...
The Chladni/cymatics canvas (`chladniCanvasRef`) now lives OUTSIDE `.unit__display-clip` (which still clips the spectrum/planet canvas as before) so its glow can bleed ~75px past the ring itself, sized via `.unit__viz-chladni { inset: -75px; ... }`. **Gotcha:** `.unit__viz`'s base rule sets `width/height: 100%`, which per spec overrides inset-based stretch sizing — without an override the canvas just shifts diagonally by the inset amount instead of growing symmetrically on all sides. The fragment shader already discards/fades fragments past its own normalized unit circle (`edgeFade` + `discard` when `dist > 1.0`), so enlarging the canvas moves that existing falloff out to the new bigger edge — no separate CSS mask needed for the "fall away at the edges" look.

**Centering bug + real fix (Sep 10 2026):** the original override (`width: auto; height: auto;`) still measured 8px off-center via Playwright `getBoundingClientRect()` — visibly noticeable, matching the reported screenshot. Root cause is one level deeper than the first gotcha: `<canvas>` is a *replaced element*, and per spec an absolutely-positioned replaced element with `width/height: auto` AND all four `inset` sides specified is over-constrained — it falls back to the canvas's intrinsic HTML `width`/`height` attribute (350×350) instead of stretching to the inset box, and only `left`/`top` get honored while `right`/`bottom` are silently dropped, shifting the canvas down-right. Fixed by sizing explicitly against the containing block instead of relying on `auto`: `width: calc(100% + 150px); height: calc(100% + 150px);` (150 = 75px × 2 sides). Verified via Playwright measuring `.unit__viz-chladni`'s center against `.unit__display-ring`'s center — offset went from `{dx:8, dy:8}` to `{dx:0, dy:0}`.

## Vibe reading entropy fix (`utils/reading.js`, Sep 11 2026)
Repeated "read your vibe" taps within the same hour/weather/tide window were coming back not just similar but byte-identical in every line and number — confirmed via a Node repro that called `buildReading()` 6-8 times back to back and diffed the output. Two separate causes:

1. **The narrative-line "entropy" only ever moved one shared `seed` value**, scaled by three different multipliers (1, 1.37, 2.11) for the moon/time/prescription lines. Multiplying a shared scalar by different constants doesn't decorrelate it — nearby seeds still land in the same length-3 bucket for all three picks at once, and the per-call golden-ratio increment was additionally weighted down to 0.4 of its full value. Repro showed 3 of 6 calls producing an identical 3-line reading. Fixed with three fully independent low-discrepancy counters (golden ratio, √2−1, √3−1 — each getting its full per-call increment, not a fraction of it) so a step >1/3 all but guarantees a different bucket every call.
2. **Sound settings (volume/freq) had zero per-call variation at all** — 100% deterministic on moon phase/weather/tide/time-of-day, none of which meaningfully change within minutes, so immediate repeats were exact duplicates by design. Added a ±6% volume / ±3% freq per-call jitter (same call-count-driven golden-ratio spread as above, applied last) to whatever ended up active, mirroring the jitter approach `randomizeActive()` already uses elsewhere in `App.jsx`.

**Follow-up (Sep 11 2026, later same day): the displayed frequency reason text was still frozen** even after the jitter fix above — every "new"/"waningCrescent" moon-phase reading's `reasons.pink` literally said "136.1 Hz" no matter what, because that text is written inline in the moon-phase `switch` *before* `tideFreqBias`/jitter run, using the raw `OM_HZ`/`WU_YIN_HZ.*` constant rather than the final adjusted value. All 8 noise-frequency reason strings across the switch now use a `{FREQ}` placeholder instead of the literal constant/template value; after tide bias + jitter settle the real `noise[k].freq`, a pass right after the jitter loop substitutes the placeholder with that final number. Separately: this reading legitimately recurs pink+chime+birds across a session right now because the moon really is near-new (`moonPhase()` = 0.976, past the `>0.97` new-moon threshold) and the weather really is clear at the test location — not a bug, the grounding logic is working as designed.

**Location was also never a direct entropy input** — it only affected the reading indirectly through whatever weather numbers came back for that spot, so two different locations with coincidentally identical weather produced identical readings. `fetchWeather()` now also returns `lat`/`lon` (was `{wc,wind,precip,aqi}`, now includes coords), and `buildReading()` folds them into all three narrative seeds via `locEntropy`. Verified via Playwright against the live app (4 consecutive "read your vibe" opens, all 4 came back with distinct 3-line combinations) and a production build (`npm run build`) to confirm no syntax issues from the rewrite.

## Deploy race condition fix (`.github/workflows/deploy.yml`, Sep 11 2026)
Two pushes ~8 seconds apart (a docs-only "dump:" commit, then the actual "fix:" code commit right after — the standard pattern this project's `/dump` runs produce every time) triggered overlapping `deploy-dev` jobs with no concurrency control. Both ran `aws s3 sync dist/ ... --delete` against the same bucket; the older commit's job (whose own build still hashed to the *previous* JS bundle, since it touched no code) finished ~1 second after the newer one and re-synced the bucket back to that older state — deleting the newer commit's just-uploaded JS bundle. `index.html` was left referencing a hash that no longer existed in S3, so every browser load got an S3/CloudFront error response served as HTML in place of the JS module — the site went fully blank ("Failed to load module script... MIME type of text/html").

**Fixed with a workflow-level `concurrency: { group: deploy-${{ github.ref }}, cancel-in-progress: true }`** at the top of `deploy.yml`, applying to both `deploy-main` and `deploy-dev`. A new push now cancels any still-running deploy for the same branch instead of letting two race to sync last. Given this project's `/dump` workflow always pushes a docs commit immediately followed by a code commit, **this race could recur on `main`** (the `deploy-main` job) under the same two-commits-close-together pattern if a `main` push sequence happens quickly — the fix covers both jobs, but watch for it.
