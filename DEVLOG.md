...
- **Origin blur on center-spawned ripples**: with the console background gone, the ripple shapes (halo/flower/star) read as too sharp right where they spawn, directly behind the controls. They now draw with a `ctx.filter: blur()` that's strong at birth (near the console's center) and fades to fully sharp by ~90px out, without needing to reintroduce any console background
