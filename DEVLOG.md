...
- Confirmed via `gh run list --json ... startedAt,updatedAt` showing the overlap, then via direct `curl` against the live site: `index.html` referenced `index-BRGgSF15.js`, but fetching that path returned `content-type: text/html` (a stale/deleted asset caught by the CDN's error fallback) instead of JS
- **Fixed with a workflow-level `concurrency: { group: deploy-${{ github.ref }}, cancel-in-progress: true }`** in `deploy.yml` — a new push now cancels any still-running deploy for the same branch instead of letting two race to sync last. Pushing this fix itself (with no other deploy in flight) resynced the bucket cleanly: verified `index.html` → `index-D6V40kr4.js` now 200s as `text/javascript`, and a full Playwright pass against the live site (page load + opening "read your vibe") showed zero console/page errors
- **Lesson for future rapid-fire `/dump` sessions**: each `/dump` run here does a docs-only commit *then* a separate code commit seconds apart, which is exactly the push cadence that triggered this race. The concurrency fix should prevent recurrence, but worth watching after any future back-to-back pushes
...
- **Circle-viz got a "faux glass" lens treatment**: `.unit__display-ring::before` adds a fixed diagonal highlight gradient that slowly breathes in and out (9s cycle, opacity 0.35↔0.85) rather than a traveling sweep like the console's own shimmer, so the ring reads as curved glass. Verified the animation is actually running via computed-style sampling over time, not just present in the CSS
- **Cymatics layer now bleeds ~75px past the ring** instead of being clipped flush at its edge — moved the Chladni canvas out of `.unit__display-clip` (which still clips the spectrum/planet canvas as before) and enlarged it via `inset:-75px`. The shader already discards/fades past its own normalized unit circle (`edgeFade`+`discard` in the fragment shader), so enlarging the canvas just moves that existing falloff out to the new bigger edge instead of needing a second CSS-level fade. Hit a real bug getting here: `.unit__viz`'s `width/height:100%` overrides inset-based stretch sizing, so the canvas initially just shifted diagonally instead of growing symmetrically — fixed with an explicit `width/height:auto` override
- **Astro chart (`VibeAstro.jsx`) gained two features requested mid-session**: (1) classic aspect lines (conjunction/sextile/square/trine/opposition, angular separation within an orb) connecting planets currently in aspect, colored soft-blue (harmonious) vs warm-red (tense) vs neutral (conjunction); (2) dismissable by clicking anywhere that isn't the physical console (`e.target.closest('.unit')`) — previously only the same footer icon or the upper-right × could close it. Verified both via Playwright: a background click closed the overlay, and console interactivity underneath is unaffected since the check explicitly excludes `.unit`
- **New `/shapes` skill + first digest**, requested mid-session: a domain-research agent for shape/form-to-sound perception (bouba/kiki, Chladni's Law, Lissajous figures, motion/easing), registered into `/learn`'s roster alongside webdev/astro/weather/tone/divination/evolution. First digest saved to LIFE memory, emailed, and synced to cat.obfusco.us — flagged bouba/kiki as an unused calm-vs-tense visual vocabulary for presets and Lissajous figures as a cheap literal-interval-ratio visualizer idea (added to ROADMAP)

## Sep 3 2026 (1) — quieter tone-knob adjustment sound; stop-button opacity pass on a review branch
...
- **Extracted `PLANETS`/`PLANET_QUALITY`/`eclipticLon` out of `App.jsx` into new `utils/planets.js`** (also gained `ZODIAC_SIGNS`, 12 sign name+glyph pairs) so the new wheel and the existing circle-viz planet glyphs compute from one shared source instead of two copies drifting apart. `App.jsx`'s circle-viz behavior is unchanged — same functions, same call sites, just imported now
- **No houses/Ascendant** — confirmed via full-repo grep before starting that nothing in this codebase computes sidereal time or house cusps, and the existing planet math (`eclipticLon`) is geocentric-mean, location-independent by design. The reference screenshot (a natal chart) has house wedges; this "sky right now" chart only draws the 12-sign zodiac ring + planet glyphs at their true ecliptic longitude, no houses — flagged as a real scope cut, not an oversight
- **Location dialog reuses `reading.js`'s existing `getCoords()`** (now exported) — same geolocation-with-IP-fallback-with-LA-default chain `VibeReading` already uses, triggered on mount so it only prompts once the user actually opens the chart. Lat/lon isn't fed into the current sign/degree math (doesn't need to be, see above) but is shown in the wheel's caption line alongside the local time, both as a "yes, this is really using your location" cue and groundwork if house math gets added later
- **Zodiac glyphs (♈–♓) needed a text-presentation fix**: Chromium was rendering them as colorful boxed emoji rather than plain line glyphs by default. VS15 (`︎`) alone didn't fix it inside SVG `<text>`; had to also pin `font-family: 'Apple Symbols', 'Noto Sans Symbols', 'Segoe UI Symbol', sans-serif` on `.vas__sign` — caught and fixed via a Playwright screenshot before/after, not visible from reading the code
- Verified via Playwright: opened the wheel, confirmed `.vas__overlay`'s computed `pointer-events` is `none`, screenshotted the open state (wheel + glyphs + caption all correct, console visible/legible through it), and confirmed the same-icon click closes it cleanly

## Sep 1 2026 (4) — Chladni/cymatics visualizer, the last of this session's 5-item roadmap sweep

- **Roadmap line 8**, resolved by the clarifying question to mean building the full Chladni/cymatics visualizer directly rather than a smaller intermediate "deeper pulsing" step — this entry closes both that item and roadmap's separate Chladni-visualizer line at once
- New `chladniCanvasRef` — a second `<canvas>` stacked *behind* the existing spectrum/planet canvas inside `.unit__display-clip` (`z-index: 0` vs `1`, both `position: absolute; inset: 0`), with its own WebGL context and its own `requestAnimationFrame` loop, entirely separate from the existing 2D-canvas draw loop. Purely additive by construction — didn't touch a single line of the existing spectrum-bar/planet-glyph/hit-testing code
- Fragment shader implements tone-science memory's exact formula (`cos(m·π·x)·cos(n·π·y) − cos(n·π·x)·cos(m·π·y)`), rendered as glowing nodal lines (`1 - smoothstep(0, 0.12, abs(pattern))`) rather than literal particles — mode numbers `m`/`n` come from live low-band/high-band FFT energy (reusing the same `getAnalyser()` instance the spectrum bars already read), eased toward their target each frame (0.04 lerp factor, same "ease toward target" idiom used by the drift LFOs elsewhere) so mode changes read as a morph rather than a jump-cut. Colored in the same blue/pink hue family as the existing spectrum bars so it reads as one system, alpha tied to overall volume, gated off (just clears the canvas) when nothing's playing
- Verified via Playwright: idle-state screenshot confirms the layer stays inert (no stray shader output) when `!anyOn`; two active-state screenshots two seconds apart, cropped and zoomed into just the circle-viz, both clearly show the nodal-line X-pattern behind the spectrum bars, and differ from each other — confirming the pattern is genuinely live/audio-reactive rather than a static shader that happened to render once

## Sep 1 2026 (3) — frequency-search restored via circle-viz, scoped to noise isolation

- **Roadmap line 12, the trickiest of this session's 5**: the dual-knob's outer ring — the answer to the original clarifying question — turned out to already be claimed by volume, discovered only after actually reading `DualKnob.jsx`'s zone hit-testing (outer=`onMixChange`/volume, inner=`onParamChange`/color-morph, no free zone). Went back for a second round rather than guess: circle-viz's existing global drag instead, since its left/right axis (tone rate) is inert while isolated on a noise channel — no periodic-rate concept for noise, and every tone is already stopped by `toggleIsolatePlanet` during isolation — free to repurpose without displacing anything meaningful
- `onDisplayMove` now branches: while `isolatedPlanetRef.current` is set, resolve which noise channel is isolated (exactly one is ever on during isolation, so `NOISE.find(s => noiseRef.current[s.id].on)` is unambiguous), and route `dx` to `setNoiseFreq` (clamped via newly-exported `FILTER_MIN`/`FILTER_MAX` from noise.js) instead of the usual color-morph `typeAngle`. `dy`/volume is untouched either way. Ring-tip's left/right line swaps to "search freq" during isolation
- **Verification took real work to get right** — blind-clicking around the ring to find a planet glyph kept hanging Playwright (each miss re-triggers `randomizeActive()`, and polling `.textContent()` via a Playwright locator retries for its full default timeout on a miss). Fixed by computing the glyph's exact screen position analytically — replicated `eclipticLon()`'s J2000 mean-motion formula in the test script itself, using white noise's *default* 2000 Hz (a 16.8-cent near-perfect octave match to the Sun's 126.22 Hz, no setup needed) as the target. One precise click, confirmed via the cyan isolation-pulse ring appearing in a screenshot; then dragged and confirmed via a before/after crop of the glyph's exact pixel position — it fully faded out after the drag, proving the frequency really moved far enough to break the octave match rather than the handler silently no-oping
...
- **Spectral-ecology retune** (Krause layering: Water/Fire should sit Low-mid ~200–1500 Hz, not creep toward Wind/Chimes' register): Water's stream bandpass 900→600 Hz (LFO depth 350→230, scaled proportionally so the modulation still swings the same *relative* amount), stream's static second bandpass 2000→1300 Hz, rain's bandpass 2800→1450 Hz (depth 700→360) and highpass 600→500 Hz, ocean's lowpass 650→500 Hz (nudged for separation now that stream sits closer). Fire campfire's lowpass ceiling 2200→1400 Hz. Centers/character otherwise untouched — this was explicitly "best-effort, no ear available to verify quality," so kept changes conservative and structural (pull the ranges down) rather than redesigning any element's timbre. Verified via Playwright: swept every dual-knob (11 total) through its full rotation with both water and fire on, zero console errors
- **Drift-LFO consolidation** (`audio/noise.js`): primary and paired color chains used to each roll their own drift oscillator; now they share one oscillator per noise slot, each chain still getting its own depth-scaled `driftGain` sized to its own center frequency. Halves the oscillator count whenever a paired chain is built (i.e., whenever a noise knob's color-morph is turned substantially toward its paired hue) — no change in the common case where paired is never built. Trade-off worth naming: primary and paired now wobble in lockstep (same underlying rate) instead of independently — acceptable given the modulation depth is only ±3.5%, but a real behavior change if anyone's listening closely for it
- **Audited water/fire's own node counts too** (both always run all 3 type-variants — stream/rain/ocean, candle/campfire/bonfire — simultaneously, crossfaded via gain, regardless of dial position) — did NOT touch this. It's load-bearing for the smooth continuous type-morph the dual-knob paradigm depends on; stopping/restarting the inactive variants to save CPU would reintroduce the click/latency problems that architecture was built to avoid. Also worth noting: at a knob's pure anchor positions (0°/120°/240°) the other two variants' gain weight is already mathematically exactly 0, meaning their contribution is silent even though their oscillators/filters keep running — the wasted CPU is real but isn't reducible without that click risk

## Aug 29 2026 — tone-science audit: wind gets Q-modulation, rest logged

- **Audited current synthesis against LIFE's `tone_science.md` memory** (last updated Jun 27 2026). Most of it turned out to already be applied, some in places not obvious from a quick grep: full Cousto planetary Hz table on the circle-viz planet glyphs (`App.jsx`'s `PLANETS`), 136.1 Hz OM anchor and 7.83 Hz Schumann resonance both live in `reading.js`'s `PULSE_HZ`/preset system, noise-color spectral slopes match the reference table exactly (pink/brown/blue/violet/grey), bell/gong use inharmonic FM ratios (bell's 2.756 mod ratio is literally the reference's first measured bell partial), and `Background.jsx`'s visual pulse rate is explicitly derived via the same Cousto octave-division method the memory describes for Schumann-as-LFO
- **One clean gap closed**: wind (`audio/tones.js` `makeWind`) modulated its bandpass frequency but never its Q, despite the memory explicitly calling Q-mod "as important as freq-mod" for this element. Added a third slow wobble-driven LFO on `bpf.Q` (base 0.7 ± 0.3, widening to ±0.5 toward the "squall" end of the Xun↔Zhen morph, so it reads as gustier resonance rather than just a moving center frequency), depth kept conservative (worst case Q≈0.2, never near instability) and rate kept slow to match wind's already-tuned "gentle, not harsh" character rather than introduce new perceptible gustiness. Verified via Playwright: wind activates with zero console errors over several seconds of the new LFOs running
- **Everything else logged to ROADMAP instead of touched**, on purpose — this was an audit, not a mandate to rewrite tuned production values: true stereo binaural beats (current pulse is amplitude-modulation, not the frequency-difference-between-ears technique the memory means by "binaural"), a Chladni/cymatics GLSL visualizer mode, a spectral-ecology band-separation pass (Water/Fire creep into Wind/Chime's register per Krause's layering rule), φ=1.618 Hz as an available pulse rate (deliberately didn't swap it into the already-multiply-tuned `PULSE_HZ` moon mapping), and fire's flicker/sub-bass rates sitting outside the memory's literal guidance (almost certainly the same intentional calming-over-realistic tradeoff already made for wind, not a bug)
